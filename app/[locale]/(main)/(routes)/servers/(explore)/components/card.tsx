...
export function Card ({
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="w-full cursor-pointer bg-foreground/10 dark:bg-background/20 rounded-[1rem] overflow-hidden"
      >
      <Link href={`/servers/${id}`}>
        ...
        <div className="bg-foreground/20 dark:bg-background/30 relative h-[11.5rem] px-[1rem]">
          <div className="absolute -top-8  left-4 w-16 h-16 flex rounded-[1rem] p-1 bg-muted">
            {avatarImage ? (
              <Image
                alt="avatar"
                src={avatarImage}
